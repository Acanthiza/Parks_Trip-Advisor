...
```{r data}
  ...
  dat <- dir_info("data") %>%
    ...
    dplyr::mutate(User = gsub(" R.$","",User)
                  ...
                  , VisType = if_else(!is.na(Origin)
                                      , if_else(Country != "Australia"
                                                , "Overseas"
                                                , if_else(Country == "Australia" & Origin != "South Australia"
                                                          , "Aus"
                                                          , "SA"
                                                          )
                                                )
...
* Removing three parks from the dataset as it was felt these sites skewed the dataset (Cleland and Seal Bay give people the opportunity to interact with, and even hold, wildlife - behaviour that is not encouraged in other parks; and Adelaide Gaol is an urban heritage site that has very different characteristics to parks):
...
In total there were `r n_distinct(dat$Park)` Parks with reviews written by `r n_distinct(dat$User)` unique reviewers. The most reviews by any one reviewer was ``r dat %>% dplyr::count(User) %>% dplyr::top_n(1) %>% dplyr::pull(User)`` who made `r dat %>% dplyr::count(User) %>% dplyr::filter(n == max(n)) %>% dplyr::pull(n)` reviews, giving an average of `r dat %>% dplyr::add_count(User) %>% dplyr::filter(n == max(n)) %>% dplyr::pull(Stars) %>% mean() %>% round(1)` stars.
...
```{r model}
  ...
  if(file_exists(modFile)) {
    ...
    mod <-
      ...
      stan_glmer(
        as.formula(paste("cbind(Y,4-Y) ~ time*Gender*Age + (time|Park) + (time|VisType)")) #binomial
        , data = datMod
        , family = binomial
        #, chains = 5
        #, iter = 500
...
Figure \@ref(fig:modResult) shows a plot of the model results. Reviewers mostly gave 5 stars, irrespective of age or gender.

The results for 18-24 year old males suffer from a lack of data points (only `r datMod %>% dplyr::filter(Gender == "Male",Age=="18-24") %>% nrow()` data points over the entire data set).

```{r predict}

  # Use the model to predict results over variables of interest
  modPred <- datMod %>%
    dplyr::select(Age,Gender,time) %>%
    unique() %>%
...
```{r predict2}
  ...
  # Use the model to predict results over variables of interest
  modPred2 <- datMod %>%
    dplyr::select(Age,Gender,Park,time) %>%
    unique() %>%
...
```{r modResult2, fig.cap = "Line is mean credible estimate and shading is 95% credible intervals. Dots are original data points (jittered for clarity)"}
  ...
  ggplot(data=modRes2,aes(Year,Mean,fill=Park)) +
    ...
    facet_wrap(~Park
               , labeller = labeller(Park = label_wrap_gen(25))
               ) +
...
```{r predict3}
  ...
  # Use the model to predict results over variables of interest
  modPred3 <- datMod %>%
    dplyr::select(Age,Gender,VisType,time) %>%
    unique() %>%
    # dplyr::group_by(Age,Gender,VisType) %>%
    # tidyr::expand(time = seq(min(datMod$time),max(datMod$time),1)) %>%
    # dplyr::ungroup() %>%
    dplyr::mutate(col = row.names(.)
                  , Y = 4
  ...
  modRes3Tab <- modRes3 %>%
          dplyr::select(VisType,Year,Mean,`Median and range in 90% credible intervals`=text) %>%
          dplyr::group_by(VisType) %>%
          dplyr::filter(Year == max(Year)) %>%
          dplyr::ungroup() %>%
          dplyr::mutate(VisType = fct_reorder(VisType,Mean,.desc = TRUE)) %>%
          dplyr::arrange(desc(Mean))
  
  kable(modRes3Tab
        , caption = paste0("Summary of model results for the last year with data. ", modRes3Tab %>% dplyr::slice(1:2) %>% pull(VisType) %>% vec_to_sentence(), " have the highest ratings (based on mean credible estimates)")
        )
...
```{r modResult3, fig.cap = "Line is mean credible estimate and shading is 95% credible intervals. Dots are original data points (jittered for clarity)"}
  ...
  ggplot(data=modRes3,aes(Year,Mean,fill=VisType)) +
    geom_line() +
    geom_ribbon(aes(ymin = modci90lo, ymax = modci90up), alpha = 0.5) +
    geom_jitter(data = datMod
                , aes(Year,Stars)
...
```{r dataWordOrigin}
  ...
  datForPlot <- datWord %>%
    ...
    dplyr::mutate(reviews = n_distinct(id)) %>%
    dplyr::ungroup() %>%
    dplyr::count(id, word, VisType, reviews, sentiment, sort = TRUE) %>%
    dplyr::count(word, VisType, reviews, sentiment, sort = TRUE) %>%
    dplyr::group_by(VisType,sentiment) %>%
    dplyr::top_n(10) %>%
    dplyr::ungroup() %>%
    dplyr::arrange(n) %>%
    dplyr::mutate(order = row_number()
                  ...
                  , sentiment = fct_explicit_na(sentiment, na_level = "neutral")
                  , sentiment = fct_relevel(sentiment, "positive", "negative")
                  , label = paste0(VisType,". reviews = ",reviews)
...
```{r wordVisType, fig.height = 12, fig.cap = "Top ten words in each sentiment class by origin of reviewer (positive, negative and neutral)"}

  ggplot(datForPlot, aes(word,per,fill=sentiment)) +
    geom_bar(stat = "identity") +
    facet_wrap(~VisType, ncol = 3) +
    # scale_x_continuous(breaks = datForPlot$order
    #                    , labels = datForPlot$word
    #                    , expand = c(0,0)
    #                    ) +
...
```{r datactivity}
  ...
  activitiesVisType <- dat %>%
    dplyr::add_count(VisType) %>%
    dplyr::group_by(VisType,n) %>%
    tidyr::nest() %>%
    dplyr::mutate(activities = map(data
                                   ,function(x) searches %>%
                                     dplyr::filter(Type == "activity") %>%
...
## Activities by origin

Figure \@ref(fig:activityByOrigin) shows how frequently each activity was mentioned depending on the origin of the reviewer.

```{r activityByOrigin, fig.height = 14, fig.cap = "Top 10 activities - frequency within park"}

  datForPlot <- activitiesVisType %>%
    #dplyr::inner_join(activities %>% dplyr::top_n(10,reviews) %>% dplyr::select(Use)) %>%
    dplyr::group_by(Use) %>%
    #dplyr::top_n(10,reviews) %>%
    dplyr::ungroup() %>%
    dplyr::arrange(VisType,per) %>%
    dplyr::mutate(order = row_number()
                  , Use = fct_reorder(Use,per)
                  )

  ggplot(datForPlot, aes(Use,per,fill=VisType)) +
...
```{r dataFacility}
  ...
  facilitiesVisType <- facilities %>%
    dplyr::left_join(dat %>% add_count(VisType) %>% select(id,VisType,n)) %>%
    dplyr::filter(facility) %>%
    dplyr::group_by(VisType,n,Use,sentClass) %>%
    dplyr::summarise(sentences = n()
                     , reviews = n_distinct(id)
                     , sent = mean(sent)
                     ) %>%
...
## Facilities by origin

Figure \@ref(fig:facilityByOrigin) shows how frequently each facility was mentioned depending on the origin of the reviewer.

```{r facilityByOrigin, fig.height = 14, fig.cap = "Facilities - frequency by origin of reviewer"}

  datForPlot <- facilitiesVisType %>%
    #dplyr::inner_join(facilitiesVisType %>% dplyr::select(VisType,n) %>% unique() %>% dplyr::top_n(10,n)) %>%
    dplyr::arrange(VisType,per) %>%
    dplyr::mutate(order = row_number()
                  , Use = fct_reorder(Use,per)
                  )
