...
```{r modResult, fig.cap = "Model results. Most reviewers give 5 stars irrespective of age or gender. The lower 95% credible intervals go to three stars in almost every case - i.e. more than 95% of responses are three stars or more. Older males (65+ years old) - only in 2013 and 2014 were they most likely to give 5 stars"}
...
```{r modResult2, fig.cap = "Model results. "}

  ggplot(data=modRes,aes(Year,Mean)) +
    geom_line() +
    geom_ribbon(aes(ymin = modci90lo, ymax = modci90up), alpha = 0.6) +
    scale_colour_viridis_d() +
    scale_fill_viridis_d() +
    facet_wrap(~label
