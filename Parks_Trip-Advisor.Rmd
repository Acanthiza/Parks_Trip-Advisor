...
These three analyses answer most of the questions posed by the community engagement branch. A further analysis of words used in the titles and reviews was also undertaken.

# Data exploration

## Summary

Between `r format(min(dat$Date),"%d/%B/%Y")` and `r format(max(dat$Date),"%d/%B/%Y")` there were `r nrow(dat)` reviews on TripAdvisor that could be associated with South Australian parks.
...
```{r predict2}
  ...
  kable(modRes %>%
          ...
          dplyr::arrange(Park)
        , caption = paste0("Summary of model results for the last year. ", modRes %>% dplyr::filter(Year == max(Year)) %>% dplyr::top_n(-3,Mean) %>% pull(Park) %>% vec_to_sentence(), " have the lowest stars (based on mean credible estimates)")
        )
...
```{r modResult2, fig.cap = "Model results - mean credible intervals. Belair and Point Labatt both seem to be getting increasingly negative reviews - although the mean credible estimate for both remains above 4 stars currently"}
  ...
  ggplot(data=modRes,aes(Year,Mean,fill=Park)) +
    geom_line() +
...
```{r predict3}
  ...
  kable(modRes %>%
          ...
          dplyr::arrange(State)
        , caption = paste0("Summary of model results for the last year. ", modRes %>% dplyr::filter(Year == max(Year)) %>% dplyr::top_n(2,Mean) %>% pull(State) %>% vec_to_sentence(), " give the highest stars (based on mean credible estimates)")
        )
...
```{r modResult3, fig.cap = paste0("Model results - mean credible intervals. California and the North Island of New Zealand both appear to have increased the stars they give in reviews through time, however the total number of reviews are low in each case so this result should be treated cautiously. There is very little trend in stars given by origin of reviewers for other origins")}
  ...
  ggplot(data=modRes,aes(Year,Mean,fill=State)) +
    geom_line() +
    geom_ribbon(aes(ymin = modci90lo, ymax = modci90up), alpha = 0.5) +
    scale_fill_viridis_d(end=0.9) +
    facet_wrap(~label
