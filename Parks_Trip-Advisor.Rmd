...
```{r dataTitle}
  ...
  removeWords <- c("granite","island","national","park","flinders","ranges","kangaroo","chase","victor","harbor","harbour","morialta","adelaide","mt","mount","lofty","coober","pedy","rocks","arch","gully","ki","fur","wilpena","pound","coffin")
...
```{r parkTitle, fig.height = 8, fig.cap = "Top ten words in the title text for each sentiment class (positive, negative and neutral) for the parks with the most reviews"}
...
```{r parkReview, fig.height = 8, fig.cap = "Top ten words in the review text for each sentiment class (positive, negative and neutral) for the parks with the most reviews"}
...
```{r reviewActivity, fig.cap = paste0(datForPlot %>% dplyr::top_n(3) %>% dplyr::pull(word) %>% vec_to_sentence()," are by far the most populat activities mentioned in reviews")}
...
```{r parkActivity, fig.cap = "The reviews provided for a park often spill over into surrounding activites. This explains, say, camping being an activity mentioned for Granite Island"}
