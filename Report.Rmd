...
DEW are interested to know what information is contained in TripAdvisor reviews. Specifically, the following analyses were identified by the community engagement branch as potentially useful:
...
```{r dataWord}
  ...
  datForPlot <- datWord %>%
    dplyr::count(id, Origin, reviews, word, sentiment, sort = TRUE) %>%
    dplyr::count(Origin, reviews, word, sentiment, sort = TRUE) %>%
  ...
  wordChi <- chi_square(datForPlot %>%
                          dplyr::select(Origin,word,n) %>%
                              dplyr::group_by(word) %>%
                              dplyr::filter(n > 3) %>%
                              dplyr::mutate(levels = n()) %>%
                              dplyr::ungroup() %>%
...
* `r paste0(datWord %>% dplyr::select(Title) %>% unique() %>% dplyr::filter(grepl(mostNeg,Title)) %>% dplyr::sample_n(3) %>% dplyr::pull(Title),collapse="\n* ")`

A `r if_else(wordChi$doChSqPlot,"","non-")`significant interaction was found between origin of reviewer and words used `r wordChi$chSqText`.
```{asis, eval=wordChi$doChSqPlot, echo=TRUE}
This can be seen visually in Figure \@ref(fig:wordChiPlot). The main contributions to the overall chi-squared value were:
```
`r  if_else(wordChi$doChSqPlot,paste0(wordChi$chSqVis %>% dplyr::top_n(3,contribution) %>% dplyr::arrange(desc(contribution)) %>% pull(label) %>% vec_to_sentence(";"),"."),"")`
...
```{r neutral}
  ...
  kable(datWord %>%
          dplyr::filter(word %in% fixSentiment) %>%
...
```{r wordChiPlot, fig.height = 10, eval = wordChi$doChSqPlot, fig.cap = paste0("Large contributions to the significant chi-squared value were: ",wordChi$chSqVis %>% dplyr::group_by(direction) %>% dplyr::top_n(1,contribution) %>% dplyr::arrange(desc(contribution)) %>% pull(label) %>% vec_to_sentence(";"))}
...
```{r datactivity}
  ...
  activityChi <- chi_square(activities %>%
                              dplyr::select(Origin,Use,n=nUse) %>%
                              dplyr::group_by(Use) %>%
                              dplyr::filter(n > 3) %>%
                              dplyr::mutate(levels = n()) %>%
                              dplyr::ungroup() %>%
...
A `r if_else(activityChi$doChSqPlot,"","non-")`significant interaction was found between origin of reviewer and activities mentioned `r activityChi$chSqText`.
```{asis, eval=activityChi$doChSqPlot, echo=TRUE}
This can be seen visually in Figure \@ref(fig:activityChiPlot). The main contributions to the overall chi-squared value were:
```
`r  if_else(activityChi$doChSqPlot,paste0(activityChi$chSqVis %>% dplyr::top_n(3,contribution) %>% dplyr::arrange(desc(contribution)) %>% pull(label) %>% vec_to_sentence(";"),"."),"")`
...
```{r activityChiPlot, fig.height = 10, eval = activityChi$doChSqPlot, fig.cap = paste0("Large contributions to the significant chi-squared value were: ",activityChi$chSqVis %>% dplyr::group_by(direction) %>% dplyr::top_n(1,contribution) %>% dplyr::arrange(desc(contribution)) %>% pull(label) %>% vec_to_sentence(";"))}

  activityChi$chSqPlot
...
```{r dataFacility}
  ...
  facilityChi <- chi_square(facilities %>%
                              dplyr::select(Origin,Use,n=nUse) %>%
                              dplyr::group_by(Use) %>%
                              dplyr::filter(n > 3) %>%
                              dplyr::mutate(levels = n()) %>%
...
A `r if_else(activityChi$doChSqPlot,"","non-")`significant interaction was found between origin of reviewer and facilities mentioned `r activityChi$chSqText`.
```{asis, eval=activityChi$doChSqPlot, echo=TRUE}
This can be seen visually in Figure \@ref(fig:activityChiPlot). The main contributions to the overall chi-squared value were:
```
`r  if_else(activityChi$doChSqPlot,paste0(activityChi$chSqVis %>% dplyr::top_n(3,contribution) %>% dplyr::arrange(desc(contribution)) %>% pull(label) %>% vec_to_sentence(";"),"."),"")`
...
```{r facilityChiPlot, fig.height = 10, eval = facilityChi$doChSqPlot, fig.cap = paste0("Large contributions to the significant chi-squared value were: ",facilityChi$chSqVis %>% dplyr::group_by(direction) %>% dplyr::top_n(1,contribution) %>% dplyr::arrange(desc(contribution)) %>% pull(label) %>% vec_to_sentence(";"))}

  facilityChi$chSqPlot
...
```{r dataWildlife}
  ...
  wildlifeChi <- chi_square(wildlife %>%
                              dplyr::select(Origin,Use,n=nUse) %>%
                              dplyr::group_by(Use) %>%
                              dplyr::filter(n > 3) %>%
                              dplyr::mutate(levels = n()) %>%
...
A `r if_else(wildlifeChi$doChSqPlot,"","non-")`significant interaction was found between origin of reviewer and wildlife mentioned `r wildlifeChi$chSqText`.
```{asis, eval=wildlifeChi$doChSqPlot, echo=TRUE}
This can be seen visually in Figure \@ref(fig:wildlifeChiPlot). The main contributions to the overall chi-squared value were:
```
`r  if_else(wildlifeChi$doChSqPlot,paste0(wildlifeChi$chSqVis %>% dplyr::top_n(3,contribution) %>% dplyr::arrange(desc(contribution)) %>% pull(label) %>% vec_to_sentence(";"),"."),"")`
...
```{r wildlifeChiPlot, fig.height = 10, eval = wildlifeChi$doChSqPlot, fig.cap = paste0("Large contributions to the significant chi-squared value were: ",wildlifeChi$chSqVis %>% dplyr::group_by(direction) %>% dplyr::top_n(1,contribution) %>% dplyr::arrange(desc(contribution)) %>% pull(label) %>% vec_to_sentence(";"))}

  wildlifeChi$chSqPlot
...
There was a statistical difference found in the frequency with which certain words were mentioned, depending on the origin of the reviewer (also see Figures \@ref(fig:wordOverall) and \@ref(fig:wordChiPlot)):

  * `r paste0(wordChi$chSqVis %>% dplyr::select(per,label) %>% dplyr::top_n(3,per) %>% dplyr::pull(label),collapse = "\n* ")`
...
There was a statistical difference found in the frequency with which certain activities were mentioned, depending on the origin of the reviewer (also see Figures \@ref(fig:activityOverall) and \@ref(fig:activityChiPlot)):

  * `r paste0(activityChi$chSqVis %>% dplyr::select(per,label) %>% dplyr::top_n(3,per) %>% dplyr::pull(label),collapse = "\n* ")`
...
There was a statistical difference found in the frequency with which certain facilities were mentioned, depending on the origin of the reviewer (also see Figures \@ref(fig:facilityOverall) and \@ref(fig:facilityChiPlot)):

  * `r paste0(facilityChi$chSqVis %>% dplyr::select(per,label) %>% dplyr::top_n(3,per) %>% dplyr::pull(label),collapse = "\n* ")`
...
There was a statistical difference found in the frequency with which certain wildlife were mentioned, depending on the origin of the reviewer (also see Figures \@ref(fig:wildlifeOverall) and \@ref(fig:wildlifeChiPlot)):

  * `r paste0(wildlifeChi$chSqVis %>% dplyr::select(per,label) %>% dplyr::top_n(3,per) %>% dplyr::pull(label),collapse = "\n* ")`
