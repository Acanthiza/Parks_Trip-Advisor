...
This data analysis and report writing was done in a single scripted workflow (script file: [`r list.files(here::here(),pattern="Report.Rmd",recursive=TRUE)`](`r paste0("https://github.com/Acanthiza/",project,"/",list.files(here::here(),pattern="t.Rmd",recursive=TRUE))`) using the programs 'R' and 'R-studio'.
...
```{r dataTitle}
  ...
  removeWords <- c("granite","island","national","park","flinders","ranges","kangaroo","chase","victor","harbor","harbour","morialta","adelaide","mt","mount","lofty","coober","pedy","rocks","arch","gully","ki","wilpena","pound","coffin")
  ...
  activities <- c("walk","run","cycle","drive","camp","nature","wildlife","heritage","fishing")
...
Figure \@ref(fig:title) shows the ten most common positive, negative and neutral words given in the `Title` field. The presence of `worth` in this list suggested that reviewers thought that the positive sentiments they expressed came at a cost (perhaps time and/or distance). A random sample of contexts in which `worth` occured:

* `r paste0(datTitle %>% dplyr::filter(word == "worth") %>% dplyr::sample_n(5) %>% dplyr::pull(Title),collapse="\n* ")`

Figure \@ref(fig:review) shows the ten most common positive, negative and neutral words given in the `Review` field. Again, the most frequently occuring words are positive or neutral. Also, `worth` occurs frequently again, but so does `easy`. A random sample of contexts in which `easy` occured:

* `r paste0(datReview %>% dplyr::filter(word == "easy") %>% dplyr::sample_n(5) %>% dplyr::pull(Review),collapse="\n* ")`
...
```{r review, fig.cap = "Reviews used words with positive sentiment more frequently than words with negative sentiment"}
  ...
  ggplot(datForPlot, aes(word,per,fill=sentiment)) +
    ...
    facet_grid(sentiment~., scales = "free_y", space = "free_y") +
    scale_fill_viridis_d(end = 0.8) +
    labs(y = "Percentage of reviews using word (or synonym)")

```

## By park sentiment

Figure \@ref(fig:parkTitle) shows, by park, the five most common positive, negative and neutral words given in the `Title` field. The lowest frequency at which `nature` occurs is third. A random sample of contexts in which `nature` occurs:

* `r paste0(datTitle %>% dplyr::filter(grepl("nature",Title)) %>% dplyr::sample_n(5) %>% dplyr::pull(Title),collapse="\n* ")`

Figure \@ref(fig:parkReview) shows, by park, the five most common positive, negative and neutral words given in the `Review` field. Each park had `hard` in the negative words, followed by `difficult` which occured in most parks. Some examples of the context in which `hard` occurs:

* `r paste0(datReview %>% dplyr::filter(word == "hard") %>% dplyr::sample_n(3) %>% dplyr::pull(Review),collapse="\n* ")`

```{r parkTitle, fig.height = 10, fig.cap = "Top ten words in the title text for each sentiment class (positive, negative and neutral) for the parks with the most reviews"}
...
```{r parkReview, fig.height = 10, fig.cap = "Top ten words in the review text for each sentiment class (positive, negative and neutral) for the parks with the most reviews"}
...
Figure \@ref(fig:reviewActivity) shows how frequently various activities were mentioned in the `Review` field. This information is essentially the same as in Figure \@ref(fig:review) but with a focus only on activities.

Figure \@ref(fig:parkActivity) shows the frequency with which activities are mentioned in the `Review` field for the most reviewed parks. Often reviewers mention things done outside the park in their reviews. For example, the reviews for Granite Island that mentioned camping gave lots of information about the area around that park:

* `r paste0(datReview %>% dplyr::filter(Park == "Granite Island", word == "camp") %>%  dplyr::pull(Review),collapse="\n* ")`

---

```{r reviewActivity, fig.cap = paste0(datForPlot %>% dplyr::top_n(3) %>% dplyr::pull(word) %>% vec_to_sentence()," are by far the most popular activities mentioned in reviews")}

  datForPlot <- datReview %>%
    dplyr::filter(word %in% activities) %>%
    dplyr::count(reviews,word,id) %>%
    dplyr::count(reviews,word) %>%
    dplyr::mutate(word = fct_reorder(word,n)
                  , per = 100*n/reviews
                  )

  ggplot(datForPlot, aes(word,per)) +
    geom_col() +
    coord_flip() +
    labs(y = "Percentage of reviews using word (or synonym)")
...
```{r parkActivity, fig.height = 8, fig.cap = "The reviews provided for a park often spill over into surrounding activites. This explains, say, camping being an activity mentioned for Granite Island"}
  ...
  datForPlot <- datReview %>%
    ...
    dplyr::filter(reviews > quantile(unique(reviews),probs=0.5)) %>%
    dplyr::filter(word %in% activities) %>%
    dplyr::count(Park,word,reviews,id) %>%
    dplyr::count(Park,word,reviews) %>%
    dplyr::mutate(word = fct_reorder(word,n)
                  , per = 100*n/reviews
                  , label = paste0(Park,". reviews = ",reviews)
                  ) %>%
  ...
  ggplot(datForPlot, aes(order,per)) +
    geom_bar(stat = "identity") +
    facet_wrap(~label, scales = "free_y", labeller = labeller(label = label_wrap_gen(20))) +
    scale_x_continuous(breaks = datForPlot$order
                       , labels = datForPlot$word
                       , expand = c(0,0)
                       ) +
...
Reviewers most frequently mention `r datReview %>% dplyr::filter(is.na(sentiment)) %>% dplyr::count(word) %>% dplyr::top_n(3,n) %>% dplyr::pull(word) %>% vec_to_sentence(";")` in their reviews along with positive words such as `r datReview %>% dplyr::filter(sentiment == "positive") %>% dplyr::count(word) %>% dplyr::top_n(5,n) %>% dplyr::pull(word) %>% vec_to_sentence(";")`.
